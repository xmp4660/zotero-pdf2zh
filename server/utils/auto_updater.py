...
REMOTE_VERSION_FILES = ("server/server.py", "server/runtime.py")


def _resolve_runtime_root_path(root_path=None):
    if root_path:
        return root_path
    try:
        import runtime

        runtime_root = getattr(runtime, "root_path", None)
        if runtime_root:
            return runtime_root
    except Exception:
        pass
    # server/utils/auto_updater.py -> server
    return os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def _resolve_local_version(local_version=None):
    if local_version:
...
def _normalize_check_inputs(local_version, update_source):
    # 兼容 check_for_updates("gitee") 这种旧调用
    if local_version in ("github", "gitee") and update_source == "github":
        return None, local_version
    return local_version, update_source


def _remote_file_url(update_source, relative_path):
    if update_source == "gitee":
        return f"https://gitee.com/guaguastandup/zotero-pdf2zh/raw/main/{relative_path}"
    return f"https://raw.githubusercontent.com/guaguastandup/zotero-pdf2zh/main/{relative_path}"


def _fetch_remote_version(update_source):
    for relative_path in REMOTE_VERSION_FILES:
        remote_file_url = _remote_file_url(update_source, relative_path)
        try:
            with urllib.request.urlopen(remote_file_url, timeout=30) as response:
                remote_content = response.read().decode("utf-8")
            match = VERSION_PATTERN.search(remote_content)
            if match:
...
def perform_update_optimized(root_path=None, local_version=None, expected_version=None, update_source='github'):
    """
    ...
    Args:
        ...
        update_source: 更新源，'github' 或 'gitee'
    """
    root_path = _resolve_runtime_root_path(root_path)
    local_version = _resolve_local_version(local_version)
...
def check_for_updates(local_version=None, update_source='github'):
    """
    从 GitHub 检查是否有新版本。
    ...
    """
    local_version, update_source = _normalize_check_inputs(local_version, update_source)
    local_version = _resolve_local_version(local_version)
    if not local_version:
        print("⚠️ [自动更新] 无法确定本地版本号, 已跳过.\n")
        return None

    print("🔍 [自动更新] 正在检查更新...")
    try:
        remote_version = _fetch_remote_version(update_source)
