...
新版本v3.0.37 | [旧版本v2.4.3](./2.4.3%20version/README.md)

**v3.0.37 更新内容：**
- ✨ 新增超时配置和进度显示功能
- ✨ 服务端支持异步任务处理，避免长时间翻译导致连接超时
- ✨ 翻译完成后自动弹出通知提醒
- 🔧 优化进度窗口显示，支持实时进度更新
...
-   A：
    ...
    -   检查防火墙是否开启
    -   关闭杀毒软件并重启电脑
    -   **[v3.0.37新增]** 如果翻译大文件时出现超时，可以在插件设置中调整"请求超时时间"（默认30分钟）
    -   **[v3.0.37新增]** 确保服务端已更新到最新版本，新版服务端支持异步处理，可避免长时间翻译导致的连接超时
